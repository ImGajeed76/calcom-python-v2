...
class BookingOutput20240813(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["id", "uid", "title", "description", "hosts", "status", "cancellationReason", "reschedulingReason", "rescheduledFromUid", "start", "end", "duration", "eventTypeId", "eventType", "attendees", "guests", "meetingUrl", "location", "absentHost", "bookingFieldsResponses"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['cancelled', 'accepted', 'rejected', 'pending', 'rescheduled']):
            raise ValueError("must be one of enum values ('cancelled', 'accepted', 'rejected', 'pending', 'rescheduled')")
        return value
